...
![](Input/whydemographymatters.jpg){.absolute top="130" left="550" width="500"}

![](Input/proyeccionesypoliticas.jpg){.fragment top= width="535"}

![](Input/appliedmathematical.jpg){.fragment top="50" left="415" width="525"}

![](Input/preston.jpg){.fragment top="50" left="415" width="560"}

![](Input/rowland.jpg){.fragment top="50" left="415" width="560"}

![](Input/swanson.jpg){.fragment top="50" left="415" width="590"}

![](Input/biodemography.jpg){.fragment top="50" left="415" width="585"}

![](Input/salgueiro.jpg){.fragment top="50" left="415" width="585"}

![](Input/watcher.jpg){.fragment top="50" left="415" width="585"}

![](Input/poston.jpg){.fragment top="50" left="415" width="585"}

![](Input/human.png){.fragment top="50" left="415" width="585"}

![](Input/tools.png){.fragment top="50" left="415" width="680"}
...
-   Simplicidad: Más fácil de administrar porque solo considera dónde están las personas el día del censo.
...
-   Precisión en la planificación de recursos: Proporciona datos más precisos para la asignaci ́on de recursos y la planificación de políticas públicas, reflejando la residencia permanente de la población.
-   Representación exacta: Mejora la precisión en la representación política y distribución de distritos electorales.
-   Datos estables: Ayuda a obtener una imagen más estable y menos fluctuante de la población , útil para la planificación a largo plazo.
...
-   Complejidad y costos: Más difícil y costoso de administrar debido a la necesidad de verificar la residencia habitual.
...
-   Campañas de concientización pública
-   Uso de tecnología.
...
datatable(base_mx, 
  ...
  # Add custom style to change the font size of table cells
  htmlwidgets::prependContent(
    tags$style(HTML("
      table.dataTable {
        font-size: 20px;  /* Font size for table headers and cells */
      }
      table.dataTable td {
        font-size: 20px;  /* Font size for table data cells */
      }
      div.dataTables_wrapper div.dataTables_length label,
      div.dataTables_wrapper div.dataTables_filter label,
      div.dataTables_wrapper div.dataTables_info {
        font-size: 20px;  /* Font size for 'Show entries', 'Search', and 'Showing 1 to ...' */
      }
      .dataTables_wrapper .dataTables_paginate {
        font-size: 12px;  /* Font size for pagination */
      }
    "))
  )
```
...
#| code-fold: false
 tabla_izq  <- tabla_izq %>%
  mutate(across(c(prop), ~ round(.x, 3)))
datatable(tabla_izq, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
base_mx_pror  <- base_mx_pror %>%
  mutate(across(c(pob_fin), ~ round(.x, 3)))
...
<!--Bloque 1-->
::: {style="font-size: 1.2em; color: gray; margin-top: 10px;"}
 Índice de Whipple, usando la tabla de datos `base_mx_pror`
:::
Tabla izquierda
```{r}
#| echo: true
...
#| code-fold: false
tabla_izq  <- tabla_izq %>%
  mutate(across(c(pob_num), ~ round(.x, 3)))
datatable(tabla_izq, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
tabla_der  <- tabla_der %>%
  mutate(across(c(pob_den), ~ round(.x, 3)))
datatable(tabla_der, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
tabla  <- tabla %>%
  mutate(across(c(pob_num,pob_den), ~ round(.x, 3)))
...
::: {style="font-size: 1.2em; color: gray; margin-top: 10px;"}
 Índice de Whipple, usando la tabla de datos `base_mx_pror`
:::
Aqui mostramos el resultado del cálculo del índice de Whipple **W**
```{r}
#| echo: true
#| code-fold: true
library(tidyverse)
...
#| code-fold: false
tabla  <- tabla %>%
  mutate(across(c(pob_num,pob_den,W), ~ round(.x, 3)))
datatable(tabla, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
-   Ponderar las sumas de los pasos 1 y 2 y sumar los resultados para obtener una población combinada (por ejemplo, pesos 1 y 9 para el dígito 0; pesos 2 y 8 para el dígito 1).
...
#| code-fold: false
tabla_izq  <- tabla_izq %>%
  mutate(across(c(step1), ~ round(.x, 3)))
datatable(tabla_izq, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
tabla_izq  <- tabla_izq %>%
  mutate(across(c(step1,tot_pop1), ~ round(.x, 3)))
datatable(tabla_izq, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
tabla_der  <- tabla_der %>%
  mutate(across(c(step2), ~ round(.x, 3)))
datatable(tabla_der, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
tabla_der  <- tabla_der %>%
  mutate(across(c(step2,tot_pop2), ~ round(.x, 3)))
datatable(tabla_der, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
tabla_izq <- base_mx_pror %>% 
  ...
  mutate(digit = age %% 10) %>% 
  group_by(digit) %>% 
  summarise(step1 = round(sum(pob),3), .groups = "drop") %>% 
  mutate(
    weights  = c(1:10),
    tot_pop1 = step1*weights)
tabla_der <- base_mx_pror %>% 
  ...
  filter(!age %in% c(10:19)) %>% 
  group_by(digit) %>% 
  summarise(step2 = round(sum(pob),3), .groups = "drop") %>% 
  mutate (
    weights = c(9:0),
    tot_pop2 = step2*weights)
...
myers_tab   <- myers_tab %>%
  mutate(across(c(step1,step2,tot_pop2), ~ round(.x, 3)))
...
#| code-fold: false 
myers_tab  <- myers_tab %>%
  mutate(across(c(tot_pop1,tot_pop2), ~ round(.x, 3)))
datatable(myers_tab, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
myers_tab  <- myers_tab %>%
  mutate(across(c(tot_pop1,tot_pop2,blended,blend_perc,deviat), ~ round(.x, 3)))
datatable(myers_tab, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
myers_tab  <- myers_tab %>%
  mutate(across(c(myers), ~ round(.x, 3)))
datatable(myers_tab, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| code-fold: false
base_mx_5  <- base_mx_5 %>%
  mutate(across(c(pob,pob2), ~ round(.x, 3)))
datatable(base_mx_5, options = list(scrollX = TRUE, scrollY = "500px"))
```
...
#| echo: false
#| code-fold: false
base_mx_5M   <- base_mx_5M  %>%
  mutate(across(c(pob,pob2), ~ round(.x, 3)))
datatable(base_mx_5M , options = list(scrollX = TRUE, scrollY = "300px"))
```
...
#| echo: false
#| code-fold: false
base_mx_5H   <- base_mx_5H  %>%
  mutate(across(c(pob,pob2), ~ round(.x, 3)))
datatable(base_mx_5H , options = list(scrollX = TRUE, scrollY = "300px"))
```
...
tabla_M   <- tabla_M  %>%
  mutate(across(c(pob), ~ round(.x, 3)))
datatable(tabla_M , options = list(scrollX = TRUE, scrollY = "300px"))
```
...
tabla_H   <- tabla_H  %>%
  mutate(across(c(pob), ~ round(.x, 3)))
datatable(tabla_H , options = list(scrollX = TRUE, scrollY = "300px"))
```
:::
...
$r(t)=\color{red}{\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{N(t)\Delta t}}$
:::
...
::: {style="text-align: center; font-size: 1.5em; margin-top: 150px;"}
$r(t)=\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{N(t)\Delta t}=\lim_{\Delta t \to 0}\color{red}{\frac{\Delta N(t)}{\Delta t}/\frac{N(t)}{1}}$
:::
...
$r(t)=\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{N(t)\Delta t}=\color{red}{\frac{\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{\Delta t}}{N(t)}}$
:::

::: {.callout-warning title="El diablo está en los detalles"}
-   Por la ley de cocientes para los límites $\displaystyle\lim_{t\to           a}\frac{f(t)}{g(t)}=\frac{\displaystyle\lim_{t\to a}f(t)}{\displaystyle\lim_{t\to        a}g(t)}$
-   En nuestro caso especifico tenemos que $f(t)=\frac{\Delta N(t)}{\Delta t}$ y $g(t)=\frac{N(t)}{1}$
-   Notemos que $\displaystyle\lim_{\Delta t \to 0}N(t)= N(t)$, pues $N(t)$ no depende de $\Delta t$, es decir, no depende del cambio entre $t_1$ y $t_2$.
:::
...
::: {style="text-align: center; font-size: 1.5em; margin-top: 90px;"}
$r(t)=\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{N(t)\Delta t}=\frac{\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{\Delta t}}{N(t)}=\color{red}{\frac{N^{'}(t)}{N(t)}}$
:::

::: { .callout-warning title="El diablo está en los detalles"}
Por la definición de la derivada en un punto $t$ tenemos: $N^{'}(t)=\frac{dN(t)}{dt}=\displaystyle\lim_{\Delta t \to 0}\frac{N(t_2)-N(t_1)}{t_2-t_1}=\lim_{\Delta t\to 0}\frac{\Delta N(t)}{\Delta t}$
:::
...
::: {style="text-align: center; font-size: 1.5em; margin-top: 90px;"}
$r(t)=\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{N(t)\Delta t}=\frac{\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{\Delta t}}{N(t)}=\frac{N^{'}(t)}{N(t)}=\color{red}{\frac{d\, ln(N(t))}{dt}}$
:::

::: {.callout-warning title="El diablo está en los detalles"}
Por la derivada de $ln(t)$ en un punto $t$ tenemos: $\frac{d}{dt}ln(t)=\frac{1}{t}$.

\par

Y más en general, para cualquier función derivable $f$,tenemos: $\frac{d}{dt}ln[f(t)]=\frac{f^{'}(t)}{f(t)}$
:::
...
::: {style="text-align: center; font-size: 1.5em; margin-top: 90px;"}
$r(t)=\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{N(t)\Delta t}=\frac{\displaystyle\lim_{\Delta t \to 0}\frac{\Delta N(t)}{\Delta t}}{N(t)}=\frac{N^{'}(t)}{N(t)}=\frac{d\, ln(N(t))}{dt}$
:::
...
$\int_{t_1}^{t_2}r(t)dt=\int_{t_1}^{t_2}\frac{d}{dt}ln(t)=\color{red}{ln[N(t_2)]-ln[N(t_1)]}$
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$\int_{t_1}^{t_2}r(t)dt=\int_{t_1}^{t_2}\frac{d}{dt}ln(t)dt=ln[N(t_2)]-ln[N(t_1)]=\color{red}{ln[\frac{N(t_2)}{N(t_1)}]}$
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$\int_{t_1}^{t_2}r(t)dt=\int_{t_1}^{t_2}\frac{d}{dt}ln(t)dt=ln[N(t_2)]-ln[N(t_1)]=\color{red}{ln \left[ \frac{N(t_2)}{N(t_1)}\right]}$
:::
...
::: {.callout-warning title="El diablo está en los detalles"}
Esto es, pues, la exponencial, $e^x$, y el logaritmo natural, $ln(x)$, son operaciones inversas. Esto es $e^{ln(x)}=x$
:::

::: {.callout-warning title="El diablo está en los detalles"}
Ojo: $ln(e^x)=x$ si y solo si $x>0$
:::
...
::: {style="text-align: center; font-size: 0.5em; margin-top:50px;"}
$\int_{t_1}^{t_2}r(t)dt=\int_{t_1}^{t_2}\frac{d}{dt}ln(t)dt=ln[N(t_2)]-ln[N(t_1)]=ln\left[\frac{N(t_2)}{N(t_1)}\right]$
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$\ln(1+x) = \displaystyle \sum_{n=1}^{\infty} \frac{(-1)^{n-1}}{n} x^n = x - \frac{x^2}{2} + \frac{x^3}{3} - \cdots, \quad |x| < 1$
:::
Asi tenemos que:
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$\color{red}{=\frac{N(t_2)-N(t_1)}{\frac{ln\left[\frac{N(t_2)}{N(t_1)}\right]}{t_2-t_1}}}$
:::
...
:::{ .callout-warning title="El diablo está en los detalles"}
En este apartado aproximaremos $ln\left[\frac{N(t_2)}{N(t_1)}\right]$
...
* Por lo tanto $|x| = \left|\frac{N(t_2)-N(t_1)}{N(t_1)}\right|\leq \left|\frac{1}{N(t_1)}\right| = \frac{1}{N(t_1)} < 1$
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$AP[t_1,t_2]=\color{red}{\bar{N}(t_1)}.[t_2-t_1]$ donde $\bar{N}(t_1)$ es la población a mitad de periodo entre $t_1$ y $t_2$
:::

:::{ .callout-warning title="El diablo está en los detalles"}
Sabemos que: 

* $N(t_1)\leq \bar{N}(t_1)\leq N(t_2)$

* Cuando $t_1\to t_2$ tenemos que $N(t_1)\approx \bar{N}(t_1)\approx N(t_2)$
...
polygons <- data.frame(
  ...
  x = as.Date(c("1900-01-01", "1901-01-01", "1901-01-01",    # erster polygon (orange)
                ...
                "1904-01-01", "1904-01-01", "1905-01-01",    # achter
                "1904-01-01", "1905-01-01", "1905-01-01",    # neunter
                "1905-01-01", "19006-01-01","1906-01-01")),  # zehnter
  y = c(0, 0, 1, # erster
        0, 1, 1, # zweiter
        1, 1, 2, # dritter
        1, 2, 2, # vierter
...
:::
${}_{\color{green}m}q_{\color{red}n}^{\text{C}}$ probabilidad de que un individuo que pertenecea la 
cohorte $\color{blue}{C}$, que llega vivo a la $\color{red}{edad \,n}$, muera antes de alcanzar la  
edad $\color{green}m$
...
::: {.callout-note style="text-align: center; font-size: 2em; margin-top:50px;"}
### Donde
* $B_0$ Nacimientos en el año 0
* $$
...
::: {style="text-align: center; font-size: 0.7em; margin-top:30px;"}

\begin{align}

\text{tbm}=\frac{D}{N}=\frac{\displaystyle\sum_{x=0}^\omega {}_nD_x}{N}
...
=\displaystyle\displaystyle\sum_{x=0}^{\omega}\frac{{}_nD_x}{N}\frac{{}_nN_x}{{}_nN_x}  
\end{align}
...
= \displaystyle\sum_{x=0}^{\omega} \frac{{}_nD_x}{{}_nN_x} \frac{{}_nN_x}{N} 
\end{align}
:::
...
=\displaystyle\displaystyle\sum_{x=0}^{\omega} {}_nM_x \hspace{0.5cm} {}_nC_x \\
...
::: {.callout-note title="donde"}
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$\displaystyle\sum_{x=0}^{\omega} {}_nC_x = \frac{\displaystyle\sum_{x=0}^{\omega +} {}_nN_x}{N}=1$
:::
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$tbm^*=\displaystyle\sum_{x=0}^{\omega} {}_nM^{Africa}_x{}_nC^{Europa}_x$
:::
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$ASCDR^j= \displaystyle\sum_{x=0}^{\omega}  {}_nM_x^{j}\,{}_nC_x^S$
:::
para $j=A,B$ y
...
base0 <- base0_raw %>% 
  ...
  select( -c("Index", "Variant","Notes","Location code","ISO3 Alpha-code",
             "ISO2 Alpha-code","SDMX code**","Type","Parent code")) %>% 
  filter(`Region, subregion, country or area *`%in% c("World","Africa","Europe","Latin
...
base0 <- base0_raw %>% 
  ...
  filter(`Region, subregion, country or area *`%in% c("World","Africa","Europe","Latin
                                                       America and the Caribbean","Mexico")) %>% 
  gather(key = "age",value = "deaths",-`Region, subregion, country or area *`, -Year)%>% 
  rename(region = 'Region, subregion, country or area *', year = Year)
```
```{r}
#| echo: false
#| code-fold: false
...
<!-- Bloque 1 -->
::: {style="font-size: 1.2em; color: gray; margin-top: -10px;"}
Limpieza de la base de datos `WPP2024 POP F01 1 POPULATION SINGLE AGE BOTH SEXES`
:::
...
#| echo: true
library(DT)
load("Databases/base1_raw.RData")
base1 <- base1_raw
```
```{r}
#| echo: false
#| code-fold: false
datatable(head(base1,1000), options = list(scrollX = TRUE, scrollY = "500px"))
```
...
base1 <- base1_raw %>% 
  ...
  select(-c("Index", "Variant", "Notes", "Location code", "ISO3 Alpha-code", "ISO2 Alpha-code",
            "SDMX code**", "Parent code")) %>% 
  filter(Type %in% c("Region", "World"), `Region, subregion, country or area *` %in%
           c("World", "Africa", "Europe", "Latin America and the Caribbean")) 
```
```{r}
#| echo: false
#| code-fold: false
...
base1 <- base1_raw %>% 
  ...
  filter(Type %in% c("Region", "World"), `Region, subregion, country or area *` %in%
           c("World", "Africa", "Europe", "Latin America and the Caribbean")) %>% 
  gather(key = "age", value = "pob", -`Region, subregion, country or area *`, -Year, -Type)
```
```{r}
#| echo: false
#| code-fold: false
...
::: {.callout-note title="Tasa instantánea de mortalidad"}
$\mu(x)= \displaystyle \lim_{n\to 0}{}_nm_x = \lim_{n\to 0}\frac{{}_nd_x}{{}_nL_x}$
:::

## `Relaciones básicas de la mortalidad` {auto-animate="true" background-image=Background/bim1.png}

::: {.callout-note title="Tasa instantánea de mortalidad"}
$\mu(x)=\displaystyle \lim_{n\to 0}{}_nm_x = \lim_{n\to 0}\frac{{}_nd_x}{{}_nL_x} = \lim_{n\to 0}\frac{l_x-l_{x+n}}{nL_x}$
:::
...
::: {.callout-note title="Tasa instantánea de mortalidad"}
$\mu(x)= \displaystyle\lim_{n\to 0}{}_nm_x = \lim_{n\to 0}\frac{{}_nd_x}{{}_nL_x} = \lim_{n\to 0}\frac{l_x-l_{x+n}}{nL_x}=\color{red}{-\frac{dl(x)}{l(x)dx}}$
:::

::: {.callout-warning title="La trampa esta en los pormenores "}
Aplicamos la definición de la **derivada**:

$\displaystyle \lim_{n\to 0}\frac{l_x - l_{x+n}}{nl_x}=\lim_{n\to 0}-\frac{l_{x+n}-l_x}{nl_x}$
:::
...
$\displaystyle\lim_{n\to 0}\frac{l_x - l_{x+n}}{nl_x}=\lim_{n\to 0}-\frac{l_{x+n}-l_x}{nl_x}$ factorizando un signo negativo
:::
...
$\displaystyle\lim_{n\to 0}\frac{l_x - l_{x+n}}{nl_x}=\lim_{n\to 0}-\frac{l_{x+n}-l_x}{nl_x}= -\frac{1}{l_x}\lim_{x\to 0}\frac{l_{x+n-l_x}}{n}$ pues el factor $-\frac{1}{l_x}$ no depende de n y lo podemos sacar del límite.
:::
...
$\displaystyle\lim_{n\to 0}\frac{l_x - l_{x+n}}{nl_x}=\lim_{n\to 0}-\frac{l_{x+n}-l_x}{nl_x}= -\frac{1}{l_x}\lim_{x\to 0}\frac{l_{x+n-l_x}}{n}=-\frac{1}{l(x)}\frac{d}{dx}l(x)$ aplicando la definición de la **derivada**
:::
...
::: {.callout-note title="Tasa instantánea de mortalidad"}
$\mu(x)= \displaystyle\lim_{n\to 0}{}_nm_x = \lim_{n\to 0}\frac{{}_nd_x}{{}_nL_x} = \lim_{n\to 0}\frac{l_x-l_{x+n}}{nL_x}=-\frac{dl(x)}{l(x)dx}=\color{red}{-\frac{dInl(x)}{dx}}$
:::
...
::: {.callout-note title="Tasa instantánea de mortalidad"}
$\mu(x)= \displaystyle\lim_{n\to 0}{}_nm_x = \lim_{n\to 0}\frac{{}_nd_x}{{}_nL_x} = \lim_{n\to 0}\frac{l_x-l_{x+n}}{nL_x}=-\frac{dl(x)}{l(x)dx}=-\frac{dInl(x)}{dx}$
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$q(X)= \displaystyle \lim_{\Delta t \to 0}\frac{P(X\in(x,x+\Delta))}{\Delta t}$
:::
...
::: {style="text-align: center; font-size: 1em; margin-top:50px;"}
$q(X)= \displaystyle \lim_{\Delta t \to 0}\frac{P(X\in(x,x+\Delta t))}{\Delta t}$
:::
...
$\mu(X)= \displaystyle\lim_{\Delta x \to0} \frac{P(X\in (x,x+\Delta x)|Se\,llegó\, con \,vida\, al\, tiempo\,x )}{\Delta x}$
:::
